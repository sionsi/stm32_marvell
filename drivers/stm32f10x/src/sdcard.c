...
#if 1	//103暂时只支持1bit模式
#define SDIO_TRANSFER_CLK_DIV		    ((uint8_t)0x1)   //72m/(x+2)
#define ENABLE_DATA_WIDE 	0
#else
#define SDIO_TRANSFER_CLK_DIV		    ((uint8_t)0x8)   //72m/(x+2)
#define ENABLE_DATA_WIDE 	1			
#endif
...
int SD_Init(void)
{
	...
	SDIO_DeInit();
	DMA_ITConfig(DMA2_Channel4, DMA_IT_TC | DMA_IT_TE | DMA_IT_HT,DISABLE);
...
again:
	...
	///mmc_send_if_cond(MMC_VDD_32_33 | MMC_VDD_33_34);
...
int mmc_send_if_cond(u32 ocr)
{
	...
	SDIO_CmdInitStructure.SDIO_Argument =  ((ocr & 0xFF8000) != 0) << 8 | 0xaa;;
	...
	//errorstatus = CmdResp4Error();
	errorstatus = CmdResp5Error(SD_SEND_IF_COND, &result_pattern);
	if (SD_OK != errorstatus)
	{
		goto end;
	}
	//result_pattern = SDIO_GetResponse(SDIO_RESP1);
	
	p_dbg("result_pattern %x", result_pattern);
...
void SDIO_IRQHandler(void)
{
	...
	{					 
		...
		cpu_sr = local_irq_save();
		SDIO->DCTRL &= DCTRL_CLEAR_MASK;
	 	DMA2_Channel4->CCR &= CCR_ENABLE_Reset;
	 	SDIO->MASK &= SDIO_IT_SDIOIT;
